...
def get_removable_drives():
    ...
    for part in psutil.disk_partitions(all=True):
        if 'removable' in part.opts or '/media/' in part.mountpoint:
            try:
                device_path = ''.join([i for i in part.device if not i.isdigit()])
                if device_path not in [d['device'] for d in drives]:
                    with open(f'/sys/class/block/{os.path.basename(device_path)}/size') as f:
                        size_bytes = int(f.read()) * 512
                    drives.append({
                        "device": device_path,
...
def select_drive():
    ...
    if not drives:
        print_error("No USB drives found.")

    print("\nSelect a USB drive to format:")
    for i, drive in enumerate(drives):
        size_gb = drive['size'] / (1024**3)
        print(f"  {i+1}: {drive['device']} - {drive['model']} ({size_gb:.2f} GB)")

    while True:
        try:
            choice = int(input("Enter the number of the drive: ")) - 1
            if 0 <= choice < len(drives):
                return drives[choice]['device']
            else:
